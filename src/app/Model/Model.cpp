...
glm::mat4 Model::GetWorldMatrix() const
{
    ...
    // @TODO 2 - Completed here
    // Check if the model has an animation
    if (mAnimation != nullptr)
    {
        // If model has an animation, get the animation world matrix from it
        worldMatrix = (*mAnimation).GetAnimationWorldMatrix();
    }
    else
    {
        // If no animation is set, build world matrix from the position, scaling and rotation info
    ...
    // Return the world matrix depending on if animation is set or not
	return worldMatrix;
}
